...
유스케이스의 트랜잭션 경계는 함께 실행되는 코드 묶음이 아니라
사용자의 성공 기준과 요구사항으로 판단한다.
...
요구사항에 원 작업과 후속 작업의 성공 관계가 드러나 있지 않으면,
구현 전에 질문한다.

### 후속 작업 실패 처리

후속 작업 실패를 원 작업과 분리하더라도 조용히 무시하지 않는다.
로그와 재시도 또는 운영 보상 경로를 둔다.

로그에는 최소한 다음 맥락이 포함되어야 한다.
...
재시도 방식은 요구사항과 운영 전제에 따라 결정한다.
스케줄러, 상태 테이블, 이벤트 / 큐, 운영자 수동 보상 중 무엇을 사용할지는 백엔드 작업자가 임의로 정하지 않는다.
