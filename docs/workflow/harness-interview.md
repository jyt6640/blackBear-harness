...
프로젝트 하네스가 이미 있으면 전체 인터뷰를 다시 제안하지 않는다.
이 문서는 더 이상 읽을 필요가 없고, 프로젝트 하네스가 정본이다.

아래 경우에만 해당 영역의 질문으로 부분 재진입한다.

- pending으로 남긴 영역에 작업이 도달했을 때
- not-applicable 영역의 재검토 신호가 나타났을 때
- 프로젝트 규모가 커져 상위 프로파일의 질문이 필요해졌을 때
- base 하네스 버전이 갱신되었을 때
