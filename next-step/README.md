...
3. Test Agent(/test)는 `00-task-card.md`를 확인하고 `01-red-test-report.md`를 작성하고 멈춘다.
4. Feat Agent(/feat)는 `01-red-test-report.md`를 확인하고 `02-green-implementation-report.md`를 작성하고 멈춘다.
5. Refactor Agent(/refactor)는 구현을 정리하고 `03-refactor-report.md`를 작성하고 멈춘다. 개선할 것이 없어도 행위 보존 확인을 기록한다.
6. Review Agent(/review)는 산출물과 diff를 검증해 `04-review-report.md`와 `06-scorecard.md`(철학 점수표)를 작성하고 멈춘다. 반려면 사유에 따라 Feat / Refactor / Test로 되돌린다.
7. 오케스트레이터는 `05-summary.md`를 작성하고 백로그 상태를 갱신한 뒤 영구화할 지식만 docs / decisions / 커밋 메시지 / PR 설명으로 승격한다.
...
- Review 판정이 승인이고 `05-summary.md`가 작성된 작업만 history로 이동한다.
